...
const plans = [
  ...
  { name: 'PRIME PLAN',   tag: 'PRIME PLAN',   minDeposit: 1000, dailyROI: 0.010, duration: 9999 },
  { name: 'ULTRA PLAN',   tag: 'ULTRA PLAN',   minDeposit: 3000, dailyROI: 0.010, duration: 9999 },
];
...
async function seed() {
  ...
  const primePlan = seededPlans.find(p => p.name === 'PRIME PLAN');
  const ultraPlan = seededPlans.find(p => p.name === 'ULTRA PLAN');

  if (john && primePlan && ultraPlan) {
    ...
    if (!existing) {
      ...
      const endB   = new Date(startB); endB.setDate(endB.getDate() + primePlan.duration);
      const invB = await prisma.investment.create({
        data: { userId: john.id, planId: primePlan.id, amount: 5000, bonusAmount: 500, startDate: startB, endDate: endB, status: 'active', totalEarned: 1500 },
      });
      ...
      const endC   = new Date(startC); endC.setDate(endC.getDate() + ultraPlan.duration);
      const invC = await prisma.investment.create({
        data: { userId: john.id, planId: ultraPlan.id, amount: 20000, bonusAmount: 2000, startDate: startC, endDate: endC, status: 'active', totalEarned: 4000 },
      });
