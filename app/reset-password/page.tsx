...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.password.length < 8) {
      setError(t('resetPasswordLengthError') || 'Password must be at least 8 characters');
      return;
    }
    if (formData.password !== formData.confirmPassword) {
      setError(t('resetPasswordMismatchError') || 'Passwords do not match');
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="group/input">
              ...
              <input
                ...
                className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 focus:border-violet-500/50 dark:focus:border-[#00FF88]/50 focus:ring-violet-500/50 dark:focus:ring-[#00FF88]/50 rounded-xl px-4 py-3.5 text-slate-900 dark:text-[#FFFFFF] placeholder-slate-400 dark:placeholder-white/20 focus:outline-none focus:ring-1 focus:bg-white/10 transition-all duration-300 font-mono text-sm"
            ...
            <div className="group/input">
              ...
              <input
                ...
                value={formData.code}
                onChange={handleChange}
                className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 focus:border-violet-500/50 dark:focus:border-[#00FF88]/50 focus:ring-violet-500/50 dark:focus:ring-[#00FF88]/50 rounded-xl px-4 py-3.5 text-slate-900 dark:text-[#FFFFFF] placeholder-slate-400 dark:placeholder-white/20 focus:outline-none focus:ring-1 focus:bg-white/10 transition-all duration-300 font-mono text-sm tracking-widest"
                placeholder="123456"
                required
              />
            </div>
            ...
            <div className="group/input">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={handleChange}
                  className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 focus:border-violet-500/50 dark:focus:border-[#00FF88]/50 focus:ring-violet-500/50 dark:focus:ring-[#00FF88]/50 rounded-xl pl-4 pr-12 py-3.5 text-slate-900 dark:text-[#FFFFFF] placeholder-slate-400 dark:placeholder-white/20 focus:outline-none focus:ring-1 focus:bg-white/10 transition-all duration-300 font-mono text-sm"
                  placeholder="••••••••••••"
                  required
                />
                <button
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full mt-4 px-4 py-4 bg-slate-900 dark:bg-white text-white dark:text-black hover:bg-slate-800 dark:hover:bg-gray-100 font-semibold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 group/btn relative overflow-hidden disabled:opacity-50"
            >
              <span className="relative z-10 font-mono tracking-widest text-sm uppercase">
                {isLoading ? 'Updating...' : t('resetPasswordUpdateBtn')}
              </span>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0F1419] text-slate-900 dark:text-white flex flex-col items-center justify-center relative p-4 transition-colors duration-250">
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in zoom-in-95 duration-500">
        <Suspense fallback={
          <div className="bg-white dark:bg-[#0A0F14]/60 p-10 rounded-2xl text-center text-sm font-mono text-slate-400">
            Loading...
          </div>
        }>
          <ResetPasswordForm />
