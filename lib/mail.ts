...
export async function sendVerificationEmail(email: string, firstName: string, code: string): Promise<boolean> {
  ...
  try {
    const fromEmail = process.env.RESEND_FROM_EMAIL || 'Vexta Auth <onboarding@resend.dev>';
    const { error } = await resend.emails.send({
      from: fromEmail,
      to: email,
      subject: 'Verify your Vexta Account',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #f0f0f0; rounded: 12px;">
          <h2 style="color: #00D9FF; font-weight: 300;">Welcome to <span style="font-weight: 700;">VEXTA</span></h2>
          <p>Hi ${firstName},</p>
          <p>Thank you for registering with Vexta. To verify your email address, please use the following 6-digit verification code:</p>
          ...
          <p style="color: #808A9D; font-size: 12px; margin-top: 30px;">
            This verification code is valid for 1 hour. If you did not register for a Vexta account, please ignore this email.
          </p>
        </div>
      `,
    });
